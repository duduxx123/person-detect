...
# 先给 Ultralytics 的 BoT-SORT 打补丁，让它在底层关联阶段真正使用我们自己的 ReID 编码器。
install_custom_botsort_reid(REID_WEIGHTS_PATH)
yolo = YOLO(MODEL_PATH)
# 外层仍然保留一个独立的 ReID 编码器，用于“track_id 已经断了之后”的旧 UUID 认回。
reid_encoder = PersonReIDEncoder(REID_WEIGHTS_PATH)
...
while True:
    ...
    # 第 1 层：YOLO + BoT-SORT 负责每一帧的检测与短时关联。
    # 这里返回的 track_id 是底层轨迹编号，短时比较稳，但长时间丢失后仍可能变化。
    results = yolo.track(
        source=frame,
        persist=True,
        classes=[0],
    ...
    for result in results:
        ...
        for box, track_id, conf, cls_id in zip(boxes, track_ids, confs, cls_ids):
            ...
            # 先裁出当前人的图像区域，后面既用于显示，也用于 ReID 编码。
            ...
            if track_id not in track_uuid_map:
                # 当底层出现新 track_id 时，不急着给它一个全新的 UUID，
                # 而是先去最近丢失的人里做一次“认回旧身份”尝试。
                ...
                if reused_uuid is not None:
                    ...
                    if match_debug is not None:
                        ...
                        log_track_event(f"new_uuid track_id={track_id} -> uuid={track_uuid_map[track_id]} reason=no_lost_candidate")
            ...
            # 维护每个 UUID 的历史特征库，让后续重识别不只依赖最后一帧。
    ...
    # 第 3 层：把这一帧消失掉的轨迹转移到 lost_tracks。
    # 后面如果又出现一个新 track_id，就有机会从这里认回旧 UUID。
    ...
    # 定期清理过期的 lost_tracks，避免历史身份无限堆积
    for person_uuid, info in list(lost_tracks.items()):
        if now_ts - info["last_seen"] > LOST_UUID_TTL_SECONDS:
            lost_tracks.pop(person_uuid, None)
